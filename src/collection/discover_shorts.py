...
discover_shorts.py
------------------
Shorts-First Creators — channels whose primary output is YouTube Shorts.

Shorts represent YouTube's fastest-growing content format and its direct
competition with TikTok. Shorts-first creators may have fundamentally different
growth dynamics, audience demographics, and monetization patterns than
long-form creators.

Uses the videoDuration=short search parameter to find channels uploading
short-form content, then fetches full channel details.
...
Usage:
    python -m src.collection.discover_shorts [--test] [--limit N]
...
CHECKPOINT_PATH = config.STREAM_DIRS["shorts_first"] / ".discovery_checkpoint.json"

# Time windows — more windows for the larger 50K target
NUM_WINDOWS = 24
WINDOW_DAYS = 15
...
def setup_logging() -> None:
    ...
    log_file = config.LOGS_DIR / f'discover_shorts_{config.get_date_stamp()}.log'
...
def get_time_windows(num_windows: int = NUM_WINDOWS) -> List[tuple]:
    """Generate time windows (15-day chunks going back from now)."""
    windows = []
    now = datetime.utcnow()
...
def discover_shorts_channels(
    ...
    """
    Discover channels that upload Shorts.

    Searches across multiple time windows using videoDuration=short to find
    channels creating short-form content. Writes incrementally after each window.
    ...
    for idx, (pub_after, pub_before, window_label) in enumerate(windows):
        ...
        try:
            ...
            channel_details = get_channel_full_details(
                ...
                stream_type="shorts_first",
                discovery_language="global",
                discovery_keyword="videoDuration=short"
            )
...
def main():
    """Main entry point for Shorts-First Creators collection."""
    parser = argparse.ArgumentParser(description="Shorts-First Creators Discovery")
    ...
    logger.info("=" * 60)
    logger.info("SHORTS-FIRST CREATORS DISCOVERY")
    logger.info("=" * 60)
    ...
    try:
        ...
        output_path = config.get_output_path("shorts_first", "initial")

        channels = discover_shorts_channels(
            youtube=youtube,
