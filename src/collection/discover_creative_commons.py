"""
discover_creative_commons.py
----------------------------
Creative Commons Educators — channels publishing under CC licenses.

CC-licensed content represents a philosophically different approach to content
creation: sharing vs. monetizing. This stream captures the educational/altruistic
corner of YouTube that may have different relationships with AI tools (using AI
for teaching vs. for production value).

Uses the videoLicense=creativeCommon search parameter to find channels publishing
open-access content, then fetches full channel details.
...
CHECKPOINT_PATH = config.STREAM_DIRS["creative_commons"] / ".discovery_checkpoint.json"

# Time windows — moderate count for 10-15K target
NUM_WINDOWS = 12
WINDOW_DAYS = 30
...
def setup_logging() -> None:
    ...
    log_file = config.LOGS_DIR / f'discover_creative_commons_{config.get_date_stamp()}.log'
...
def discover_creative_commons_channels(
    youtube,
    ...
    """
    Discover channels publishing Creative Commons content.

    Searches across multiple time windows using videoLicense=creativeCommon
    to find channels with CC-licensed videos. Writes incrementally after each window.
    ...
    for idx, (pub_after, pub_before, window_label) in enumerate(windows):
        ...
        try:
            ...
            channel_details = get_channel_full_details(
                ...
                stream_type="creative_commons",
                discovery_language="global",
                discovery_keyword="videoLicense=creativeCommon"
            )
...
def main():
    """Main entry point for Creative Commons Educators collection."""
    parser = argparse.ArgumentParser(description="Creative Commons Educators Discovery")
    parser.add_argument('--test', action='store_true', help='Run in test mode (50 channels)')
    parser.add_argument('--limit', type=int, default=15000, help='Target channel count')
    ...
    logger.info("=" * 60)
    logger.info("CREATIVE COMMONS EDUCATORS DISCOVERY")
    logger.info("=" * 60)
    ...
    try:
        ...
        output_path = config.get_output_path("creative_commons", "initial")

        channels = discover_creative_commons_channels(
            youtube=youtube,
        ...
        # Topic distribution (interesting for CC — expect heavy Education)
        by_topic: Dict[str, int] = {}
        for ch in channels:
            topic = ch.get('topic_1', 'None')
            by_topic[topic] = by_topic.get(topic, 0) + 1

        logger.info("  By primary topic:")
